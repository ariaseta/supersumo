...
const route = getRouteApi('/_authenticated/vps/')

type DataTableProps = {
  data: VPS[]
}

export function VPSList({ data }: DataTableProps) {
  ...
  const table = useReactTable({
    ...
    globalFilterFn: (row, _columnId, filterValue) => {
      ...
      return name.includes(searchValue) || ip.includes(searchValue)
    },
  ...
  return (
    ...
    >
      <DataTableToolbar
        table={table}
        searchPlaceholder='Filter by name or IP...'
        filters={[
          {
            columnId: 'status',
            title: 'Status',
